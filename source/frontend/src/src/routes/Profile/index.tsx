...
const ProfileBase = withNavigate(withMenu(({ navigate, menu, page, profile, classname, value, setBlocked, setFriendship }: ProfileBaseProps) => {
...
export default protectedRoute({ authenticated: true, fallback: '/signin' }, withParams(withUser(class Profile extends React.Component<ProfileProps> {
	...
	async onJoinProfile() {
		...
		this.setProtectedState({ profile: result });
	}
	...
	setBlocked() {
		...
		delete profile.friendship;
		profile.blocked = true;

		this.setProtectedState({ profile });
	}
