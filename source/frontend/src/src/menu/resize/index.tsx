...
export default withMenu(withUser(class Resize extends React.Component<ResizeProps> {
	...
	componentDidMount() {
		...
		image.onload = () => {
			if (image.width / image.height < schema_rect.width / schema_rect.height)
			{
				height = image.height * (schema_rect.width / image.width);
				width = image.width * (schema_rect.width / image.width);
			}
			else
			{
				height = image.height * (schema_rect.height / image.height);
				width = image.width * (schema_rect.height / image.height);
			}
	...
	onSave(e: any) {
		...
		if (!this.drag_current || !this.schema_current || !this.data)
			return null;
		...
		const crop_left = (schema_rect.left - drag_rect.left) / drag_rect.width * this.data.width;
		const crop_top = (schema_rect.top - drag_rect.top) / drag_rect.height * this.data.height;
		const crop_width = schema_rect.width / drag_rect.width * this.data.width;
		const crop_height = schema_rect.height / drag_rect.height * this.data.height;

		context.drawImage(this.image_current, crop_left, crop_top, crop_width, crop_height, 0, 0, canvas.width, canvas.height);
	...
	/* getters */
	private get data() {
		return this.state.data as HTMLImageElement;
	}
