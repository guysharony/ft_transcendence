...
export class UsersService {
  ...
  async set42User(profile: Profile): Promise<UserEntity> {
    const user = this.usersRepository.create({
      ...
      email_42: profile.emails[0].value,
    });
  ...
  async findOneBy42Email(email_42: string) {
    return await this.usersRepository.findOne({ where: { email_42 } });
  }
  ...
  async findLike(query: string): Promise<UserEntity[]> {
    let data = this.usersRepository.createQueryBuilder("user").take(10);
