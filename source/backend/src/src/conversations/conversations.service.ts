...
import * as escape from "@/common/utils/escape";
...
export class ConversationsService {
  ...
  async findLike(query: string): Promise<ConversationEntity[]> {
    if (query.length < 1 || query.charAt(0) != "#") return [];

    return await this.conversationsRepository
      .createQueryBuilder("conversations")
      .take(10)
      .where("conversations.name like :name", {
        name: `${escape.like(query.slice(1))}%`,
      })
      .andWhere(
        "(conversations.visibility = 'public' OR conversations.visibility = 'protected')"
      )
      .select([
  ...
  async searchInvitableUsers(conversation: ConversationEntity, query: string) {
    ...
    if (query.length < 1 || query.charAt(0) != "@")
      data = data.where("user.display_name like :display_name", {
        display_name: `${escape.like(query)}%`,
      });
    else
      data = data.where("user.username like :username", {
        username: `${escape.like(query.slice(1))}%`,
      });
  ...
  async quite(conversation: ConversationEntity, user: UserEntity) {
    ...
    const sockets = await this.conversationMembersRepository.find({
      where: {
        conversation: {
          id: conversation.id,
        },
    ...
    const single_user = sockets.length < 2;
