...
export class ConnectionsController {
  ...
  async connections(
    ...
    const current = req.currentUser as UserEntity;
  ...
  async addFriend(
    ...
    if (data.source && data.source.id == target.id && data.type == 3) {
      return res.status(201).send({ blocked: true });
    }
  ...
  async removeFriend(
    ...
    if (!data || !(data instanceof ConnectionEntity))
      return res.status(201).send({});
