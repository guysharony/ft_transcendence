...
export class ConnectionService {
  ...
  async answerToFriendRequest(source: number, target: number, type: 1 | 2) {
    ...
    if (data.source.id == source && data.type == 3) return null;
  ...
  async removeBlock(source: UserEntity, target: UserEntity) {
    ...
    const deleted = await this.connectionRepository.delete({
      id: friendship.id,
    });

    const is_blocked = await this.connectionRepository.findOne({
      where: { source: { id: target.id }, target: { id: source.id }, type: 3 },
      relations: ["source", "target"],
    });
    if (is_blocked) return is_blocked;
